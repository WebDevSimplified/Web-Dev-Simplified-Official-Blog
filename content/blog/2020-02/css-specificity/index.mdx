...
*PS: This article is available in video form as well. Check it out if you are visual learner. [Click here to view the video.](https://youtu.be/CHyPGSpIhSs)*

*PPS: There is an interactive specificity calculator at the bottom of the page you can checkout [here](#calculator)*.
...
This selector would have a class specficity of two and an id specificity of one.
...
If after checking `!important`, ids, and classes, the specificity is still the same then the count of all element selectors is used. This count includes all HTML element selectors as well as all psuedo selectors such as `::before`. The count of all these elements goes into the fourth and final number in the array. For example the following selector would have an element specificity of one.
