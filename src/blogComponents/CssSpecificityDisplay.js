  ...
  return (
    <div style={WRAPPER_STYLES}>
      <div style={CONTAINER_STYLES}>
        ...
        <div style={NUMBER_WRAPPER_STYLES}>
          <div style={{ ...NUMBER_STYLES, backgroundColor: ID_COLOR }}>{idCount}</div>
          <div style={NUMBER_DESCRIPTOR_STYLES}>#id</div>
        </div>
        <div style={NUMBER_WRAPPER_STYLES}>
          <div style={{ ...NUMBER_STYLES, backgroundColor: CLASS_COLOR }}>{classCount}</div>
          <div style={NUMBER_DESCRIPTOR_STYLES}>.class</div>
        </div>
        <div style={NUMBER_WRAPPER_STYLES}>
          <div style={{ ...NUMBER_STYLES, backgroundColor: ELEMENT_COLOR, marginRight: 0 }}>{elementCount}</div>
...
function selectorDetailsDisplay(selectorDetails) {
  ...
  return Object.entries(selectorDetails.reduce((result, detail) => {
    return { ...result, [detail.type]: [...(result[detail.type] || []), detail.selector] }
  }, {})).sort(([a], [b]) => a - b).map(([type, parts]) => {
    let color
    ...
    return (
      <div style={{ ...DETAILS_STYLES, backgroundColor: color }} key={type}>
        {parts.map((part, index) => <div key={index}>{index + 1}. <span style={{ paddingLeft: '.35rem' }}>{part}</span></div>)}
      </div>
    )
  })
}